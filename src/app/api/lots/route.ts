...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    if (!lotId || typeof body.deletionRequested !== 'boolean') {
      return NextResponse.json({ error: 'lotId and deletionRequested are required' }, { status: 400 })
    }

    const lot = await db.lot.findUnique({ where: { id: lotId } })
    if (!lot) return NextResponse.json({ error: 'Lot not found' }, { status: 404 })

    // Clients can only mark their own lots for deletion — they can never
    // permanently delete (see DELETE below, admin-only).
    ...
    const { emitRealtime, REALTIME_EVENTS } = await import('@/lib/realtime')
    emitRealtime({ event: REALTIME_EVENTS.LOT_UPDATED, data: { lotIds: [lotId] } })

    return NextResponse.json({ lot: updated })
  } catch (e) {
    console.error('[lots] PATCH FATAL:', e)
    return NextResponse.json({ error: 'Server error' }, { status: 500 })
  }
}
...
export async function DELETE(req: Request) {
  ...
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  if (user.role !== 'ADMIN') {
    return NextResponse.json({ error: 'Only admin can permanently delete lots' }, { status: 403 })
  }
  ...
  try {
    ...
    // WonLot and DeliveryRequest both cascade-delete at the DB level
    // (onDelete: Cascade in schema.prisma), so no manual pre-cleanup needed.
