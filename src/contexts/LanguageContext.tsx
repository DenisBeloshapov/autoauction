...
const translations: Record<Language, Record<string, string>> = {
  ru: {
    ...
    "delivery.ownerFullNamePlaceholder": "Иванов И.И.",
    ...
    "delivery.searchPlaceholder": "Поиск по номеру кузова или лота (несколько — через запятую)",
  ...
  en: {
    ...
    "delivery.searchPlaceholder": "Search by body number or lot (multiple — comma-separated)",
...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  // Initialize synchronously from localStorage (client-only) to avoid SSR mismatch
  // and avoid the setState-in-effect anti-pattern.
  ...
  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem("aa_lang", lang);
  }, [lang]);
