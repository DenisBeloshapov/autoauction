...
export function ClientPanel() {
  ...
  const markForDeletion = async (id: string) => {
    if (!(await confirm(t("lots.markDeletionConfirm"), { danger: true, confirmLabel: t("lots.markForDeletion") }))) return;
    const snapshot = lots;
    setLots((prev) => prev.map((l) => (l.id === id ? { ...l, deletionRequested: true } : l)));
    const res = await fetch("/api/lots", {
      method: "PATCH",
      headers: authHeaders,
      body: JSON.stringify({ lotId: id, deletionRequested: true }),
    });
    if (res.ok) {
      toast.success(t("lots.markedForDeletion"));
    } else {
      setLots(snapshot);
      const data = await res.json().catch(() => ({}));
            ...
            <div className={`grid grid-cols-1 ${group.lots.length > 1 ? "md:grid-cols-2" : ""} gap-4`}>
              {group.lots.map((lot) => (
                <motion.div
                  ...
                  initial={{ opacity: 0, y: 8 }}
                  animate={{ opacity: 1, y: 0 }}
                  className={`aa-card aa-card-hover p-5 ${lot.deletionRequested ? "!border-destructive/60" : ""}`}
                >
