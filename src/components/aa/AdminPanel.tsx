...
type Lot = {
  ...
  wonLot: { id: string; price: number | null; status: string; bodyNumber: string | null; vesselName: string | null; loadingDate: string | null } | null;
};
...
export function AdminPanel() {
  ...
  return (
    ...
    >
                    ...
                    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="aa-card overflow-hidden">
                      <div className="overflow-x-auto scroll-slim">
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {list.map((lot) => (
                              <motion.tr key={lot.id} variants={itemVariants} onClick={() => setDetailLot(lot)} className={`border-t border-border/60 hover:bg-muted/30 transition cursor-pointer ${lot.deletionRequested ? "bg-destructive/5 outline outline-1 outline-destructive/40 -outline-offset-1" : ""}`}>
                                ...
                                <td className="px-4 py-3"><button onClick={(e) => { e.stopPropagation(); deleteLot(lot.id); }} className="text-xs text-destructive hover:underline flex items-center gap-1 transition"><Trash className="w-3 h-3" /></button></td>
                              </motion.tr>
                            ))}
                          </tbody>
                        </table>
      ...
      {/* Compact lot detail — clicking a lot in a client's list (Клиенты) */}
      <Modal open={!!detailLot} onClose={() => setDetailLot(null)} title={detailLot ? `#${detailLot.lotNumber}` : ""} size="sm">
        {detailLot && (
          <div className="space-y-2.5">
            <div className="flex items-center gap-2 flex-wrap">
              <StatusBadge
                status={detailLot.wonLot ? detailLot.wonLot.status : detailLot.status}
                label={t(detailLot.wonLot ? `wonStatus.${detailLot.wonLot.status}` : `status.${detailLot.status}`)}
              />
              {detailLot.deletionRequested && (
            ...
            <InfoRow icon={<FileText className="w-3.5 h-3.5" />} label={t("lots.rawText")} value={stripLotNumber(detailLot.rawText) || "—"} />
            {detailLot.comment && <InfoRow icon={<FileText className="w-3.5 h-3.5" />} label={t("lots.comment")} value={detailLot.comment} />}
            {detailLot.wonLot && (
              <>
                <InfoRow icon={<Coins className="w-3.5 h-3.5" />} label={t("delivery.price")} value={detailLot.wonLot.price != null ? `${detailLot.wonLot.price.toLocaleString()} ${t("wonLots.currency")}` : "—"} mono />
                <InfoRow icon={<FileText className="w-3.5 h-3.5" />} label={t("wonLots.bodyNumber")} value={detailLot.wonLot.bodyNumber || "—"} />
                {(detailLot.wonLot.vesselName || detailLot.wonLot.loadingDate) && (
                  <>
                    <InfoRow icon={<Boat className="w-3.5 h-3.5" />} label={t("delivery.vesselName")} value={detailLot.wonLot.vesselName || "—"} />
                    <InfoRow icon={<Calendar className="w-3.5 h-3.5" />} label={t("delivery.loadingDate")} value={detailLot.wonLot.loadingDate || "—"} />
                  </>
                )}
              </>
            )}
        ...
        return (
          <motion.div key={group.key} variants={itemVariants} className="aa-card aa-card-hover overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              {group.lots.map((lot) => (
                <div key={lot.id} className={`flex items-start justify-between gap-3 py-2 ${lot.deletionRequested ? "rounded-md border border-destructive/60 px-2.5 -mx-2.5" : ""}`}>
    ...
    <div>
              ...
              <motion.div key={wl.id} variants={itemVariants} className="aa-card aa-card-hover overflow-hidden">
                ...
                <div className="px-5 pb-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2.5">
                    ...
                    <InfoRow icon={<User className="w-3.5 h-3.5" />} label={t("delivery.client")} value={wl.lot.client.name || wl.lot.client.username} />
                    {(wl.vesselName || wl.loadingDate) && (
                      <>
                        <InfoRow icon={<Boat className="w-3.5 h-3.5" />} label={t("delivery.vesselName")} value={wl.vesselName || "—"} />
                        <InfoRow icon={<Calendar className="w-3.5 h-3.5" />} label={t("delivery.loadingDate")} value={wl.loadingDate || "—"} />
                      </>
                    )}
                  </div>
                  {isDuty && (
        ...
        footer={
          <>
            ...
            <button onClick={submitContract} disabled={savingContract} className="h-10 px-4 rounded-md bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 disabled:opacity-50 transition-colors flex items-center gap-1.5">
              {savingContract && <CircleNotch className="w-4 h-4 animate-spin" weight="bold" />}
              {t("common.save")}
            </button>
          </>
        }
      >
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t("delivery.vesselName")}</label>
            <input
              type="text"
              value={vesselName}
              onChange={(e) => setVesselName(e.target.value)}
              placeholder={t("delivery.vesselNamePlaceholder")}
              autoFocus
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t("delivery.loadingDate")}</label>
            <input
              type="date"
              value={loadingDate}
